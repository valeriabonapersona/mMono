'
  Script: Main results males at rest
  Input: output of analysis_males.R
  Output: Figure 4 paper
...
for(i in unique(males_mod_enough$out_mono)) {
  print(i)
  
  fig_mono_main[[i]] <- males_mod_enough %>%
    filter(out_mono == i) %>%
    mutate(
